...
df_group_1 = (
    df.groupby(["Date", "Item Name", "Year"])
...
g_1 = (
    alt.Chart(df_group_1)
    .mark_bar(color="maroon")
    .encode(
        alt.X("yearmonth(Date):T", axis=alt.Axis(title="Year", tickCount=5)),
        alt.Y(
            "sum(Sales):Q",
            axis=alt.Axis(
                title="Total Sales", labelColor="maroon", titleColor="maroon"
            ),
        ),
...
g_2 = (
    alt.Chart(df_group_1)
    .mark_line(color="black", interpolate="monotone", strokeDash=[10, 10])
    .encode(
        alt.X("yearmonth(Date):T", axis=alt.Axis(title="Date", tickCount=5)),
        alt.Y(
            "sum(Quantity):Q",
            axis=alt.Axis(
                title="Total Items Purchased", labelColor="black", titleColor="black"
            ),
        ),
...
point_1 = g_2.mark_circle(color="black").encode(opacity=alt.value(1))


# combine Total Sales and total Items Purchased
sales_purch = (
    alt.layer(g_1, g_2 + point_1)
...
g_3 = (
    ...
    .encode(
        ...
        alt.Y(
            "sum(Quantity):Q",
            axis=alt.Axis(
                title="Total Items Purchased", labelColor="maroon", titleColor="maroon"
            ),
        ),
...
point_2 = g_4.mark_circle(color="black").encode(opacity=alt.value(1))


# combine number of purchases and Average price across months
purch_price = (
    alt.layer(g_3, g_4 + point_2)
    .resolve_scale(y="independent")
    .properties(
...
g_5 = (
    ...
    .encode(
        alt.X("yearmonth(Date):T", axis=alt.Axis(title="Date")),
        alt.Y(
            "distinct(Item Name):O",
            axis=alt.Axis(
                title="Product Purchased", labelColor="maroon", titleColor="maroon"
            ),
        ),
...
prod_price = (
    alt.layer(g_5, g_4 + point_2)
    .resolve_scale(
        y="independent",
    )
    .properties(
        title="Product Purchased and Average Price Across Months", height=300, width=300
    )
    .interactive()
    .add_selection(ad_selec)
    .transform_filter(ad_selec)
