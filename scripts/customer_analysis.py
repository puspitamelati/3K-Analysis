...
df_group_2 = df.groupby(['Date', 'Customer ID', 'Year']
...
#Top 5 Best Customer
group_2 = df.groupby(['Year','Customer ID']
            ).agg({'Quantity' : 'sum', 'Sales' : 'sum'}
            ).reset_index()
group_2['Rank'] = group_2.groupby(['Year'])['Quantity'].rank(ascending=False, method='first')

best = alt.Chart(group_2).mark_bar(color='maroon').encode(
    alt.X('sum(Quantity):Q',
         axis = alt.Axis(title = 'Date')),
    alt.Y('Customer ID:O', sort ='-x',
...
g_1 = alt.Chart(df_group_2).mark_bar(color='maroon').encode(
    alt.X('yearmonth(Date):T',
         axis = alt.Axis(title = 'Date')),
    alt.Y('distinct(Customer ID):O',
...
).properties(title='Total Customer Accros Month ',
...
group_cust = df_group_2.drop_duplicates(subset=['Customer ID'], keep='first') #new dataset that have not duplicatet customer id

g_2 = alt.Chart(group_cust).mark_bar(color='maroon', interpolate='monotone').encode(
    alt.X('yearmonth(Date):T',
         axis = alt.Axis(title = 'Date')),
    alt.Y('count(Customer ID):O',
        axis = alt.Axis(title = 'Total New Customer', titleColor='maroon')),
    tooltip = ('yearmonth(Date):T', 'count(Customer ID):N')
).properties(title='New Customer Accros Month ',
    height= 300, width = 300
).add_selection(ad_selec
).transform_filter(
    ad_selec)
...
g_3 = alt.Chart(df_group_2).transform_aggregate(
    monthly_count= 'count(Quantity):Q',
    groupby=['Date', 'Year']).mark_bar(color='maroon').encode(
    alt.X('yearmonth(Date):T',
         axis = alt.Axis(title = 'Date')),
    alt.Y('average(monthly_count):Q',
        axis = alt.Axis(title = 'Average Basket Size', format='.1f', titleColor='maroon')),
    tooltip = ('yearmonth(Date):T','average(monthly_count):Q' )
).properties(title='Average Customer Basket Size Accros Month ',
    height= 300, width = 300
).add_selection(ad_selec
